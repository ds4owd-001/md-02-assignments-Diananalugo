...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    ```{r}
    ggplot(data = gapminder_2007, mapping = aes(x= continent, y =lifeExp, fill= continent)) + geom_boxplot(outlier.shape = NA)
    #the link for the outlier solution: https://www.perplexity.ai/search/how-to-ignore-Mv4zQPCLStyQYK9SnF29uA?s=c
    #the data types are x and fill are for distinct groups and y for continous variable
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(data = gapminder_summary_continent_year, mapping = aes(x= year, y= lifeExp, color = continent)) + geom_line() + geom_point()
    ```
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(data = gapminder_summary_2007, mapping = aes(x=continent,y = count)) + geom_col()
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: The geom_bar() function is used to create a bargraph counting number of data points in each subdivision or category on the x axis. The two are identical because both count the number of data points in each category just in different ways.

    ```{r}
    ggplot(data = gapminder_2007, mapping = aes(x=continent)) + geom_bar()
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    Both the geom_histogram() and geom_bar() generate columns whose size is representative of the data points in different categories. The geom_bar() takes distinct groups as the x argument where as geom_histogram() takes on continous variables.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    The breaks argument specifies the size of bins to which the data points are going to be distributed. The first number will represents the "limit" of the first bin. In this case from what life expectancy should the count begin. It was specified as 40. The second represents the upper limit of the count which was 85 and the third represents the bin width. ie the very first bin is numbering entries with life expectancy between 40 and 42.5

    ```{r}
    ggplot(data = gapminder_2007, 
           mapping = aes(x = lifeExp, fill = continent)) + geom_histogram(col= "grey30", breaks = seq(40, 85, 2.5))
    ```
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    It contains the colors assigned to the different countries and continents
